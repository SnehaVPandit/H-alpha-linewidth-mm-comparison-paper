...
file_slope = open("slopes_intercepts_CaII_S_Ha_linewidth_mm_Q_EN.txt", "w")
...
file = open("correlations_CaII_S_Ha_linewidth_mm_Q_EN.txt", "w")
...
for wave_index in range(34):
    ...
    #IRT_index=np.asarray(np.load(flist_IRT[wave_index]))
    #Ha_index=np.asarray(np.load(flist_Ha[wave_index]))
    Ha_linewidths=np.asarray(np.load(flist_linew[wave_index]))
    
    index_1=S_index
    index_2=Ha_linewidths
    ...
    cbar.set_label('linewidth (Angstrom)', rotation=90, fontsize=30)
    cbar.ax.tick_params(labelsize=25)
    plt.title('Ha linewidths' %np.around(waves[wave_index], decimals=2),fontsize=25)
    plt.xticks(ticks=np.arange(0, 504, step=84),labels=np.arange(0, 24, step=4),fontsize=25)
    plt.yticks(ticks=np.arange(0, 504, step=84),labels=np.arange(0, 24, step=4),fontsize=25)
    plt.xlabel('Horizontal X direction (Mm)',fontsize=25)
    plt.ylabel('Horizontal Y direction (Mm)',fontsize=25)
    ...
    plt.title('S index vs Ha index at res: %s mm' %np.around(waves[wave_index+1], decimals=2),fontsize=25)
    plt.yticks(fontsize=25)
    plt.legend(fontsize=25)
    #plt.xlim(4.7,11.7)
    #plt.ylim(0.85,1.35)
    plt.xlabel('Ha linewidths',fontsize=25)
    plt.ylabel('S index',fontsize=25)
    
    plt.savefig('Ha_linewidths_S_index_scatter_plot_Both_res_6_plot_%s.png' %np.around(waves[wave_index+1], decimals=2))
    plt.close()
    
    print('S index vs Ha linewidths cha 3 scatter plot line saha plot kelay.')
    ...
    print('S index vs Ha linewidths che slopes ani intercepts ani correlations lihile')
...
plt.legend(fontsize=25)
plt.title('S index vs Ha linewidths correlations',fontsize=25)
plt.xlabel('ALMA wavelengths (mm)',fontsize=25)
plt.ylabel('correlation coefficient',fontsize=25)
plt.savefig('S_index_Ha_linewidths_correlations_Q_EN_plot_.png')
plt.close()
print('Ha linewidths correlations cha plot kela')
...
plt.title('S index vs Ha linewidths mm slopes',fontsize=25)
plt.xlabel('ALMA wavelengths (mm)',fontsize=25)
plt.ylabel('slopes',fontsize=25)
plt.savefig('S_index_Ha_linewidths_slopes_Q_EN_plot.png')
#plt.close()
#print('correlations cha plot kela')
print('S index vs Ha linewidths slopes cha plot kela')
