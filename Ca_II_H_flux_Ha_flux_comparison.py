...
#degraded resolution
flist_Ha = sorted(glob.glob('/mn/stornext/d18/RoCS/snehap/Ha_CaII_index_comparison/Ha_flux_LOWRES_*.npy'))
flist_CaH = sorted(glob.glob('/mn/stornext/d18/RoCS/snehap/Ha_CaII_index_comparison/CaII_H_flux_LOWRES_*.npy'))
...
file_slope = open("slopes_intercepts_CaII_H_flux_Ha_flux_mm_Q_EN.txt", "w")
...
file = open("correlations_CaII_H_flux_Ha_flux_mm_Q_EN.txt", "w")
...
for wave_index in range(34):
    ...
    index_1=np.asarray(np.load(flist_CaH[wave_index]))
    index_2=np.asarray(np.load(flist_Ha[wave_index]))
    ...
    plt.subplot(1,3,1)
    plt.imshow(index_1.T,origin='lower',vmin=np.nanpercentile((np.ndarray.flatten(index_1)),1),vmax=np.nanpercentile((np.ndarray.flatten(index_1)),99),cmap='gray')
    cbar=plt.colorbar()
    ...
    plt.xlabel('Solar X (Mm)',fontsize=25)
    plt.ylabel('Solar Y (Mm)',fontsize=25)
    plt.title('Ca II H flux',fontsize=25)
    ...
    plt.subplot(1,3,2)
    plt.imshow(index_2.T,origin='lower',cmap='gray',vmin=np.nanpercentile(np.ndarray.flatten(index_2),1),vmax=np.nanpercentile((np.ndarray.flatten(index_2)),99))
    cbar=plt.colorbar()
    #cbar.set_label('Ha_index',  fontsize=25)
    cbar.ax.tick_params(labelsize=25)
    plt.title('Ha flux', fontsize=25)
    plt.xticks(ticks=np.arange(0, 504, step=84),labels=np.arange(0, 24, step=4),fontsize=25)
    plt.yticks(ticks=np.arange(0, 504, step=84),labels=np.arange(0, 24, step=4),fontsize=25)
    plt.xlabel('Solar X (Mm)',fontsize=25)
    plt.ylabel('Solar Y (Mm)',fontsize=25)
    ...
    plt.title('Ca II H flux vs Ha flux %s.png' %np.around(waves[wave_index], decimals=2),fontsize=25)
    plt.yticks(fontsize=25)
    plt.legend(fontsize=25)
    plt.xlim(np.nanpercentile((index_2),1),np.nanpercentile((index_2),99))
    plt.ylim(np.nanpercentile((index_1),1),np.nanpercentile((index_1),99))
    plt.xlabel('Ha flux',fontsize=25)
    plt.ylabel('Ca II H flux',fontsize=25)
    
    plt.savefig('Ha_flux_CaII_H_flux_scatter_plot_Both_res_6_plot_%s.png' %np.around(waves[wave_index+1], decimals=2))
    plt.close()
...
plt.legend(fontsize=25)
plt.title('CaII H flux vs Ha flux correlations',fontsize=25)
plt.xlabel('ALMA wavelengths (mm)',fontsize=25)
plt.ylabel('correlation coefficient',fontsize=25)
plt.savefig('CaII_H_flux_Ha_flux_correlations_Q_EN_plot_.png')
plt.close()
print('Ha flux correlations cha plot kela')
...
plt.title('CaII H flux vs Ha flux mm slopes',fontsize=25)
plt.xlabel('ALMA wavelengths (mm)',fontsize=25)
plt.ylabel('slopes',fontsize=25)
plt.savefig('CaII_H_flux_Ha_flux_slopes_Q_EN_plot.png')
#plt.close()
#print('correlations cha plot kela')
print('CaII_H_flux vs Ha flux slopes cha plot kela')
